...
//# :::::::::   input data   :::::::::::::::::::::::::::
   ...
   string eigenvec = inp1.read("EIGENVEC");
   string averageq = inp1.read("AVERAGEQ");
   //# output
   string outcoord = inp1.read("OUTCOORD"); // 
   //# infomation
 ...
 */
   ...
   if(ifs.fail()){//error handling
      ...
      cerr<<"program was ended in reading file section\n";
      exit(1);
   }
   while(!ifs.eof()){
      ifs >> buf_double;
...
/* (2-2) read EIGENVEC and AVERAGEQ
 */
   cout<<endl<<"--- READING EIGENVECTOR AND AVERAGEQ FROM THE FILE --- \n";
   vector<double> refeigen;
   ifstream ifs( eigenvec.c_str() ); // ifstream cannot handle type string (filename); it must be char*
   if(ifs.fail()){//error handling
      cerr<<"ERROR: Cannot open file: "<<eigenvec<<endl;
      cerr<<"program was ended in reading file section\n";
      exit(1);
   }
   while(!ifs.eof()){
      ifs >> buf_double;
      refeigen.push_back(buf_double);
   }
   cout<<"refeigen[size()-1] = "<<refeigen[refeigen.size()-1]<<endl;
   cout<<"The dimensionality of eigen vector = "<<refeigen.size()<<endl;
   if(refeigen.size() != numofcomp){
      cout<<"ERROR: read data and your input parameter are different\n";
      cout<<"program ended in reading eigen vector"<<endl;
      return 2;
   }
   ifs.close();
   ifs.clear();
